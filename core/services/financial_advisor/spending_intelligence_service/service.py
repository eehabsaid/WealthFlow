...
class SpendingIntelligenceService(CategoriesMixin, MonthlyComparisonMixin, InsightsMixin):
    ...
    def payload(self) -> dict:
        ...
        has_uncategorized = Expense.objects.filter(owner=self.owner, category__isnull=True).exists()
        ...
        total_transactions = Expense.objects.filter(owner=self.owner).count()
