...
class SalaryService:
    @transaction.atomic
    def generate_current_month_salaries(self, owner, company_id: int = None) -> dict:
        current_year = datetime.now().year
        current_month = MONTH_ORDER[datetime.now().month - 1]

        companies = Company.objects.filter(owner=owner, is_active=True)
        if company_id:
    ...
    @transaction.atomic
    def mark_salary_paid(self, salary_id: int, mark_paid: bool, owner) -> dict:
        salary = SalaryEntry.objects.select_related("company").get(id=salary_id, company__owner=owner)
    ...
    @transaction.atomic
    def update_salary(self, salary_id: int, data: dict, owner) -> SalaryEntry:
        entry = SalaryEntry.objects.select_related("company").get(id=salary_id, company__owner=owner)
        old_paid = entry.paid
    ...
    @transaction.atomic
    def delete_salary(self, salary_id: int, owner) -> None:
        entry = SalaryEntry.objects.select_related("company").get(id=salary_id, company__owner=owner)
...
def get_current_monthly_salary(owner=None, year: int = None, month: str = None) -> float:
    """
    ...
    try:
        ...
        salary_qs = SalaryEntry.objects.all()
        company_qs = Company.objects.all()
        if owner is not None:
            salary_qs = salary_qs.filter(company__owner=owner)
            company_qs = company_qs.filter(owner=owner)

        # 1. Paid salary entry for current/specified year & month
        current_paid_entry = salary_qs.filter(year=year, month=month, paid__gt=0).first()
        if current_paid_entry:
            return float(current_paid_entry.paid)

        # 2. Check active company
        active_company = company_qs.filter(is_active=True).order_by("order", "id").first()
        if active_company:
