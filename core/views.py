...
from .models import Company, SalaryEntry, Bank, BalanceEntry, AppSettings, ExchangeRate, GoldPrice, Currency, ExpenseCategory, ExpenseSubcategory, Expense, BankCertificate, PagePermission, PAGE_PERMISSION_CHOICES, UserProfile, Notification, ReminderRule, CertificateStatus, AuditLog, DashboardPreference, UserPreference, LoginActivity
...
class LoginAPIView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            pass
        _audit(request, 'login', 'User', user.id, user.username)
...
def _audit(request, action, model_name='', object_id=None, object_repr='', changes=''):
    """Create an AuditLog entry. Silently ignores errors."""
    from .models import AuditLog, AppSettings
    try:
        if AppSettings.get('audit_enabled', 'true') != 'true':
            return
        # Check if this model category is enabled
        disabled = AppSettings.get('audit_disabled_models', '')
        if model_name and disabled and model_name in disabled.split(','):
            return
...
class NotificationListView(View):
    ...
    def post(self, request):
        """Generate reminder notifications (called on app load)."""
        if not request.user.is_authenticated:
            return JsonResponse({'error': 'Unauthorized'}, status=401)
        count = _generate_notifications(request.user)
...
def _generate_notifications(user):
    """Check upcoming certificate maturities and create notifications per active rules."""
    from .models import ReminderRule, Notification, BankCertificate, AppSettings
    from datetime import date, timedelta
    import json

    # Check if notifications feature is enabled
    if AppSettings.get('notifications_enabled', 'true') != 'true':
        return 0

    # Get user notification preferences
    try:
        prefs = user.preference
        if not prefs.notify_cert_expiry:
            return 0
    except Exception:
        pass

    rules = ReminderRule.objects.filter(is_active=True, rule_type='cert_maturity')
    today = date.today()
    created = 0

    for rule in rules:
        target_date = today + timedelta(days=rule.days_before)
        certs = BankCertificate.objects.filter(expiry_date=target_date).exclude(status='Closed')
        for cert in certs:
            # Avoid duplicate notifications
            exists = Notification.objects.filter(
                user=user,
                notif_type='cert_maturity',
                related_id=cert.id,
                is_read=False,
                created_at__date=today,
            ).exists()
            if not exists:
                bank_name = cert.bank.name if cert.bank else 'Unknown Bank'
                Notification.objects.create(
                    user=user,
                    title=f'Certificate Maturing in {rule.days_before} days',
                    message=f'{bank_name} certificate of {float(cert.amount):,.2f} expires on {cert.expiry_date}.',
                    notif_type='cert_maturity',
...
class ReminderRuleListView(View):
    def get(self, request):
        ...
        return JsonResponse({'rules': [r.to_dict() for r in rules]})
    ...
    def post(self, request):
        ...
        try:
            ...
            _audit(request, 'create', 'ReminderRule', rule.id, str(rule))
...
class ReminderRuleDetailView(View):
    def put(self, request, pk):
        ...
        try:
            ...
            rule.save()
            _audit(request, 'update', 'ReminderRule', rule.id, str(rule))
    ...
    def delete(self, request, pk):
        ...
        try:
            rule = ReminderRule.objects.get(pk=pk)
            _audit(request, 'delete', 'ReminderRule', rule.id, str(rule))
            rule.delete()
            return JsonResponse({'ok': True})
        except ReminderRule.DoesNotExist:
            return JsonResponse({'error': 'Not found'}, status=404)
...
class CertificateStatusListView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            _audit(request, 'create', 'CertificateStatus', s.id, s.name)
...
class CertificateStatusDetailView(View):
    def put(self, request, pk):
        if not request.user.is_staff:
            return JsonResponse({'error': 'Forbidden'}, status=403)
        try:
            s = CertificateStatus.objects.get(pk=pk)
            data = json.loads(request.body)
            ...
            s.save()
            _audit(request, 'update', 'CertificateStatus', s.id, s.name)
            return JsonResponse({'status': s.to_dict()})
        except CertificateStatus.DoesNotExist:
            return JsonResponse({'error': 'Not found'}, status=404)
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=400)
    ...
    def delete(self, request, pk):
        ...
        try:
            s = CertificateStatus.objects.get(pk=pk)
            _audit(request, 'delete', 'CertificateStatus', s.id, s.name)
            s.delete()
            return JsonResponse({'ok': True})
        except CertificateStatus.DoesNotExist:
            return JsonResponse({'error': 'Not found'}, status=404)
...
class AuditLogListView(View):
    def get(self, request):
        ...
        if action_f:  qs = qs.filter(action=action_f)
        if model_f:   qs = qs.filter(model_name__icontains=model_f)
        if user_f:    qs = qs.filter(username__icontains=user_f)
...
class DashboardPreferenceView(View):
    def get(self, request):
        if not request.user.is_authenticated:
            return JsonResponse({'error': 'Unauthorized'}, status=401)
        from .models import DashboardPreference, AppSettings
        pref, _ = DashboardPreference.objects.get_or_create(user=request.user)
        # Get admin-configured available widgets
        import json as _json
        available_raw = AppSettings.get('dashboard_available_widgets', '')
        if available_raw:
            try:
                available = _json.loads(available_raw)
            except Exception:
                available = _default_widgets()
        else:
            available = _default_widgets()
        return JsonResponse({
            'config': pref.to_dict(),
    ...
    def post(self, request):
        ...
        try:
            data = json.loads(request.body)
            pref, _ = DashboardPreference.objects.get_or_create(user=request.user)
            pref.config = json.dumps(data.get('config', {}))
            pref.save()
            return JsonResponse({'ok': True})
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=400)
...
def _default_widgets():
    return [
        {'id': 'kpi_salary',       'label': 'Salary KPIs',         'visible': True,  'order': 1},
        {'id': 'kpi_balance',      'label': 'Balance KPIs',        'visible': True,  'order': 2},
        {'id': 'salary_chart',     'label': 'Salary Chart',        'visible': True,  'order': 3},
        {'id': 'cert_summary',     'label': 'Certificate Summary', 'visible': True,  'order': 4},
        {'id': 'upcoming_certs',   'label': 'Upcoming Maturities', 'visible': True,  'order': 5},
        {'id': 'recent_expenses',  'label': 'Recent Expenses',     'visible': True,  'order': 6},
        {'id': 'expense_chart',    'label': 'Expense Chart',       'visible': False, 'order': 7},
    ]
...
class SalaryTrendView(View):
    def get(self, request):
        if not request.user.is_authenticated:
            return JsonResponse({'error': 'Unauthorized'}, status=401)
        from .models import SalaryEntry, Company
        from django.db.models import Sum
        company_id = request.GET.get('company_id')
        qs = SalaryEntry.objects.filter(paid__gt=0)
        if company_id:
            qs = qs.filter(company_id=company_id)
        # Group by year
        ...
        companies = [c.to_dict() for c in Company.objects.filter(is_active=True)]
...
class CertificateSummaryView(View):
    def get(self, request):
        if not request.user.is_authenticated:
            return JsonResponse({'error': 'Unauthorized'}, status=401)
        from .models import BankCertificate
        from django.db.models import Sum, Count
        from datetime import date, timedelta
        today = date.today()
        certs = BankCertificate.objects.all()
        summary = certs.aggregate(
            total_count=Count('id'),
            total_amount=Sum('amount'),
            total_interest=Sum('interest_value'),
        )
        upcoming_30 = certs.filter(
            expiry_date__gte=today,
            expiry_date__lte=today + timedelta(days=30),
        ).exclude(status='Closed')
        upcoming_90 = certs.filter(
            expiry_date__gte=today,
        ...
        return JsonResponse({
            ...
            'upcoming_30': [c.to_dict() for c in upcoming_30],
            'upcoming_90': [c.to_dict() for c in upcoming_90],
            'by_status':   by_status,
        })
...
class UserPreferenceView(View):
    def get(self, request):
        ...
        return JsonResponse({'preferences': pref.to_dict()})
    ...
    def post(self, request):
        ...
        try:
            ...
            if 'notify_cert_expiry' in data:
                pref.notify_cert_expiry = bool(data['notify_cert_expiry'])
            if 'notify_salary' in data:
                pref.notify_salary = bool(data['notify_salary'])
            if 'notify_system' in data:
                pref.notify_system = bool(data['notify_system'])
            if 'theme' in data:
...
class ChangePasswordView(View):
    def post(self, request):
        ...
        try:
            ...
            min_len = int(AppSettings.get('password_min_length', '8'))
            require_upper = AppSettings.get('password_require_upper', 'false') == 'true'
            require_digit = AppSettings.get('password_require_digit', 'false') == 'true'
            require_special = AppSettings.get('password_require_special', 'false') == 'true'

            if len(new_pass) < min_len:
                return JsonResponse({'error': f'Password must be at least {min_len} characters'}, status=400)
            if require_upper and not any(c.isupper() for c in new_pass):
                return JsonResponse({'error': 'Password must contain at least one uppercase letter'}, status=400)
            if require_digit and not any(c.isdigit() for c in new_pass):
                return JsonResponse({'error': 'Password must contain at least one digit'}, status=400)
            if require_special and not any(c in '!@#$%^&*()_+-=[]{}|;:,.<>?' for c in new_pass):
                return JsonResponse({'error': 'Password must contain at least one special character'}, status=400)

            request.user.set_password(new_pass)
            request.user.save()
            _audit(request, 'update', 'User', request.user.id, f'{request.user.username} changed password')
            return JsonResponse({'ok': True})
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=400)
...
class LoginActivityView(View):
    def get(self, request):
        if not request.user.is_authenticated:
            return JsonResponse({'error': 'Unauthorized'}, status=401)
        from .models import LoginActivity
        # Users see their own; staff sees all
        if request.user.is_staff and request.GET.get('all') == 'true':
            qs = LoginActivity.objects.select_related('user').all()[:200]
            return JsonResponse({'activities': [
                {**a.to_dict(), 'username': a.user.username}
                for a in qs
            ]})
        else:
            acts = request.user.login_activities.all()[:50]
