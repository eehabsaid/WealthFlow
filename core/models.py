...
class Notification(models.Model):
    ...
    notif_type  = models.CharField(max_length=50, choices=NOTIFICATION_TYPE_CHOICES, default='system')
    is_read     = models.BooleanField(default=False)
    link        = models.CharField(max_length=200, blank=True)  # hash route e.g. #bank-certificates
    related_id  = models.IntegerField(null=True, blank=True)    # pk of related object
    created_at  = models.DateTimeField(auto_now_add=True)
...
class ReminderRule(models.Model):
    """Configurable rule that triggers a notification N days before an event."""
    name        = models.CharField(max_length=200)
    rule_type   = models.CharField(max_length=50, choices=NOTIFICATION_TYPE_CHOICES, default='cert_maturity')
    ...
    def __str__(self):
        return f'{self.name} ({self.days_before}d before)'


# ════════════════════════════════════════════════════════════
# FEATURE 2: Certificate Status (configurable)
# ════════════════════════════════════════════════════════════

class CertificateStatus(models.Model):
    """Admin-configurable certificate status definitions."""
    name        = models.CharField(max_length=100, unique=True)
    color_hex   = models.CharField(max_length=7, default='#0d6efd')
...
class AuditLog(models.Model):
    ...
    username    = models.CharField(max_length=150, blank=True)  # snapshot in case user deleted
    ...
    def to_dict(self):
        return {
            'id':          self.id,
            'username':    self.username or (self.user.username if self.user else 'system'),
            'action':      self.action,
    ...
    def __str__(self):
        return f'{self.username} {self.action} {self.model_name} {self.object_id}'


# ════════════════════════════════════════════════════════════
# FEATURE 4: Dashboard Preferences (per user)
# ════════════════════════════════════════════════════════════

class DashboardPreference(models.Model):
    """Stores per-user widget order/visibility as JSON."""
    user    = models.OneToOneField(User, on_delete=models.CASCADE, related_name='dashboard_pref')
    config  = models.TextField(default='{}')  # JSON: {widgets:[{id,visible,order}]}
    updated_at = models.DateTimeField(auto_now=True)
...
# ════════════════════════════════════════════════════════════
# FEATURE 6: Extended User Profile (language + notification prefs)
# ════════════════════════════════════════════════════════════

class UserPreference(models.Model):
    """Per-user preferences (language, notifications, etc.)."""
    ...
    def to_dict(self):
        return {
            ...
            'notify_cert_expiry': self.notify_cert_expiry,
            'notify_salary':      self.notify_salary,
...
class LoginActivity(models.Model):
    ...
    def __str__(self):
        return f'{self.user.username} {"✓" if self.success else "✗"} {self.timestamp}'
