...
async function loadUsers(q = '', page = 1, size = 25) {
    ...
    data.users.forEach((u) => {
        ...
        const staffBadge = u.is_staff ? `<span class="badge bg-info" data-i18n="staff">${t('staff', 'Staff')}</span>` : '';
        const toggleBtnText = u.is_active ? t('btn_deactivate', 'Deactivate') : t('btn_activate', 'Activate');
    ...
    if (info) {
        const pagingText = t('pagination_showing', 'Showing {count} of {total} users — Page {page} / {pages}')
