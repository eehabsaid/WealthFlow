...
from .config import IMAGE_DOCX_WIDTH_INCHES, NODE_PDF_SCRIPT, BASE_DIR
...
class PdfRenderer:
    ...
    Why Playwright?
    We use Playwright (via html_to_pdf.js) instead of Python libraries like WeasyPrint or wkhtmltopdf because:
    1. WeasyPrint struggles with modern CSS Grid, Flexbox, and complex layouts generated by Markdown.
    2. wkhtmltopdf is deprecated and runs an ancient webkit engine.
    3. Playwright uses Chromium natively, ensuring the PDF output perfectly matches the visual HTML output in standard browsers.
    """
    def render(self, html_path, pdf_path):
        if not os.path.exists(NODE_PDF_SCRIPT):
            ...
            return
        try:
            cmd = ["node", NODE_PDF_SCRIPT, html_path, pdf_path]
