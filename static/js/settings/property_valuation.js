...
async function renderPropertyValuationSettings() {
  ...
  const providerOrder =
    settingsData?.settings?.property_valuation_provider_order ||
    "external_api,configured_market_rate";
  const externalEnabled =
    (settingsData?.settings?.property_valuation_external_enabled || "false") === "true";
  const externalUrl = settingsData?.settings?.property_valuation_external_url || "";
  const externalResultPath =
    settingsData?.settings?.property_valuation_external_result_path || "estimated_price";
...
async function savePropertyValuationSettings() {
  ...
  const providerOrder =
    document.getElementById("propertyValuationProviderOrder")?.value ||
    "external_api,configured_market_rate";
  const externalEnabled =
    document.getElementById("propertyValuationExternalEnabled")?.value || "false";
  const externalUrl = document.getElementById("propertyValuationExternalUrl")?.value?.trim() || "";
  const externalResultPath =
    document.getElementById("propertyValuationExternalResultPath")?.value?.trim() ||
    "estimated_price";
  const externalTimeout =
    document.getElementById("propertyValuationExternalTimeout")?.value?.trim() || "8";
  const externalHeadersRaw =
    document.getElementById("propertyValuationExternalHeaders")?.value?.trim() || "";
  ...
  if (raw) {
    try {
      ...
      showToast(
        t("invalid_property_valuation_rate_map", "Property valuation rate map must be valid JSON."),
        "error"
      );
      return;
    }
  ...
  if (externalEnabled === "true" && !externalUrl) {
    showToast(
      t(
        "invalid_property_valuation_external_url",
        "External URL is required when external valuation is enabled."
      ),
      "error"
    );
    return;
  ...
  if (externalHeadersRaw) {
    try {
      ...
      showToast(
        t(
          "invalid_property_valuation_external_headers",
          "External headers must be a valid JSON object."
        ),
        "error"
      );
      return;
  ...
  if (!Number.isFinite(Number(externalTimeout)) || Number(externalTimeout) <= 0) {
    showToast(
      t(
        "invalid_property_valuation_external_timeout",
        "External timeout must be a positive number."
      ),
      "error"
    );
    return;
  ...
  try {
    ...
    if (res.ok) {
      showToast(
        t("property_valuation_settings_saved", "Property valuation settings saved."),
        "success"
      );
    } else {
      showToast(
        t(
          "error_saving_property_valuation_settings",
          "Failed to save property valuation settings."
        ),
        "error"
      );
    }
    ...
    showToast(
      t("error_saving_property_valuation_settings", "Failed to save property valuation settings."),
      "error"
    );
  }
}
