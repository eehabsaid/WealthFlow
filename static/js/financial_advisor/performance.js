...
(function () {
  ...
  // Reusable Chart Component Engine with Rolling MA Series
  function buildPerformanceChart({ canvasId, label, timeseries, valueKey, labelKey, mainColor }) {
    ...
    // True rolling series per data point
  ...
  function renderGoldChart(goldObj) {
    ...
    const filtered = filterTimeseries(rawList, _goldTimeframe);
    _goldChartInstance = buildPerformanceChart({
  ...
  function renderCurrencyChart(currObj) {
    ...
    const filtered = filterTimeseries(rawList, _currencyTimeframe);
    _currencyChartInstance = buildPerformanceChart({
      canvasId: "fa-currency-performance-chart",
      label: `${_selectedCurrency}/EGP`,
      timeseries: filtered,
      valueKey: "mid_rate",
      labelKey: "date",
      mainColor: "rgba(139, 92, 246, 1)",
