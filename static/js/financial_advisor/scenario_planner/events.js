...
window.SP.attachEventListeners = function (pane) {
  ...
  pane.querySelectorAll(".sp-cmp-check").forEach((chk) => {
    chk.addEventListener("change", (e) => {
      ...
      if (e.target.checked) {
        if (!window.SP.state.selectedScenarioIds.includes(id))
          window.SP.state.selectedScenarioIds.push(id);
      } else {
  ...
  if (eventForm) {
    eventForm.addEventListener("submit", async (e) => {
      ...
      await window.SP.addScenarioEvent(
        window.SP.state.activeScenarioId,
        window.SP.state.selectedEventType,
        eventDate,
        params
      );
    });
