"use strict";
// Employment page rendering, employer tab switching, and tab content loading.
...
async function renderEmploymentPage(companyId = null) {
  ...
  const initialActionsHtml =
    initialCompany && initialCompany.is_active
      ? `
  ...
  const tabsHtml = companies
    .map((c) => {
      ...
      const colorDot = c.color_hex
        ? `<span class="nav-dot" style="background:${c.color_hex};display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:6px"></span>`
        : "";
      return `
