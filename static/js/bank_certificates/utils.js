  ...
  try {
    ...
    if (statuses.length === 0) {
      ...
      return defaultStatuses
        .map(
          (s) =>
            `<option value="${s}" ${s === (currentStatus || "Active") ? "selected" : ""}>${s}</option>`
        )
        .join("");
    } else {
      return statuses
...
function formatCertificateFrequencyLabel(value) {
  ...
  if (
    ...
    return t("freq_semi_annually", "Semi-Annually");
  if (freq === "annually" || freq === "annual" || freq === "yearly")
    return t("freq_annually", "Annually");
