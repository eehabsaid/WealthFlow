// user_management.js — User management page with search, pagination, and permissions
...
async function loadUsers(q = "", page = 1, size = 25) {
  ...
  data.users.forEach((u) => {
    ...
    const activeText = u.is_active ? t("yes", "Yes") : t("no", "No");
    const staffBadge = u.is_staff
      ? `<span class="badge bg-info" data-i18n="staff">${t("staff", "Staff")}</span>`
      : "";
    const toggleBtnText = u.is_active
      ? t("btn_deactivate", "Deactivate")
      : t("btn_activate", "Activate");
    const toggleBtnClass = u.is_active ? "warning" : "success";

    tr.innerHTML = `
            <td>${u.username}</td>
            <td>${u.email}</td>
            <td><span class="badge ${u.is_active ? "bg-success" : "bg-secondary"}" data-i18n="${u.is_active ? "yes" : "no"}">${activeText}</span></td>
            <td>${staffBadge}</td>
            <td>
                <button class="btn btn-sm btn-${toggleBtnClass} btn-toggle-active" data-id="${u.id}" data-i18n="${u.is_active ? "btn_deactivate" : "btn_activate"}">${toggleBtnText}</button>
                <button class="btn btn-sm btn-danger btn-delete" data-id="${u.id}" data-i18n="btn_delete">${t("btn_delete", "Delete")}</button>
                <button class="btn btn-sm btn-outline-primary btn-perms" data-id="${u.id}" data-i18n="permissions">${t("permissions", "Permissions")}</button>
            </td>
        `;
  ...
  if (info) {
    const pagingText = t(
      "pagination_showing",
      "Showing {count} of {total} users — Page {page} / {pages}"
    )
      .replace("{count}", data.users.length)
      .replace("{total}", data.total || data.users.length)
      .replace("{page}", data.page || 1)
      .replace("{pages}", data.num_pages || 1);
    info.textContent = pagingText;
  }
...
function attachHandlers() {
  document.querySelectorAll(".btn-toggle-active").forEach((b) => {
    b.onclick = async () => {
      ...
      const isActivate = b.textContent.trim() === t("btn_activate", "Activate");
  ...
  document.querySelectorAll(".btn-delete").forEach((b) => {
    b.onclick = async () => {
      const confirmMsg = t("confirm_delete_user", "Delete user?");
...
async function openPermissionsModal(userId) {
  ...
  const pagesData = pagesRes.ok ? await pagesRes.json() : { available_pages: [] };
...
async function refreshPermList() {
  ...
  data.permissions.forEach((p) => {
    ...
    div.innerHTML = `<div>${p.page}</div><div><button data-id="${p.id}" class="btn btn-sm btn-danger btn-del-perm" data-i18n="btn_remove">${t("btn_remove", "Remove")}</button></div>`;
...
if (searchInput) {
  searchInput.addEventListener("input", () => {
    ...
    searchTimeout = setTimeout(() => loadUsers(searchInput.value.trim(), 1, pageSize), 300);
  });
}
