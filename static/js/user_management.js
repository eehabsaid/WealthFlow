...
async function loadUsers(q='', page=1, size=25){
    ...
    if(info){ info.textContent = `Showing ${data.users.length} of ${data.total || data.users.length} users — Page ${data.page} / ${data.num_pages || 1}`; }
    const cur = document.getElementById('current-page'); if(cur) cur.textContent = String(data.page || 1);
    attachHandlers();
}
...
function attachHandlers(){
    document.querySelectorAll('.btn-toggle-active').forEach(b=>{
        b.onclick = async ()=>{
            ...
            const resp = await fetch('/api/users/' + id + '/', {method: 'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({is_active: isActivate})});
...
async function refreshPermList(){
    ...
    document.querySelectorAll('.btn-del-perm').forEach(b=> b.onclick = async ()=>{
        const id = b.dataset.id;
        const r = await fetch('/api/users/permissions/' + id + '/', {method:'DELETE'});
...
document.getElementById('perm-add').onclick = async ()=>{
    ...
    if(!page) return;
    const res = await fetch('/api/users/' + currentPermUserId + '/permissions/', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({page})});
...
if(prevBtn) prevBtn.onclick = ()=>{ if(currentPage>1) loadUsers(currentQuery, currentPage-1, pageSize); };
if(nextBtn) nextBtn.onclick = ()=>{ loadUsers(currentQuery, currentPage+1, pageSize); };
