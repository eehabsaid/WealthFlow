...
async function renderBalance() {
    ...
    const currencyCards = _currencies
        .map((cur) => `<div class="col-6 col-md-4 col-lg-2"><div class="currency-card"><div class="cur-flag">${cur.flag || "💱"}</div><div class="cur-code" data-i18n="${cur.code}">${cur.code}</div><div class="cur-amount">${fmt(totals[cur.code] || 0)}</div></div></div>`)
        .join("");
    ...
    const rows = entries
        .map((e) => `<tr><td class="local-type-field" data-type="${e.title || ""}"></td><td class="local-type-field" data-type="${e.balance_type || ""}"></td><td class="local-type-field" data-type="${e.bank_name || "_"}"></td><td><span style="background:rgba(26,110,245,.15);color:var(--accent-primary);padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">${e.currency_flag} ${e.currency_code}</span></td><td class="text-end amt-positive">${fmt(e.amount)}</td><td><button class="btn-icon" onclick="showBalanceModal(${e.id})"><i class="bi bi-pencil"></i></button><button class="btn-icon del" onclick="deleteBalanceEntry(${e.id})"><i class="bi bi-trash"></i></button></td></tr>`)
        .join("");
    ...
    mc.innerHTML = `
        ...
        <div class="kpi-card mb-4" style="text-align:center">
            ...
            <div style="margin-top:8px;color:var(--text-secondary);font-size:13px">${fmt(totalEGP)} + (${fmt(usdAmount)} * ${fmt(usdRate)}) + (${fmt(eurAmount)} * ${fmt(eurRate)}) + (${fmt(sarAmount)} * ${fmt(sarRate)}) + ((${fmt(goldGrams)} * (${fmt(gold24kSell)} + 28.5))</div>
            <div style="margin-top:8px;color:var(--text-secondary);font-size:13px">${fmt(totalEGP)} + (${fmt(usdAmount * usdRate)}) + (${fmt(eurAmount * eurRate)}) + (${fmt(sarAmount * sarRate)}) + (${fmt(goldValue)})</div>
        </div>
        ...
        <div class="kpi-card mb-4">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-top:20px;">
                <div><div class="kpi-label" data-i18n="net_worth">Net Worth</div><div class="kpi-value">${fmtpresent(grandTotal)}</div></div>
                <div><div class="kpi-label" data-i18n="cash_egp">Cash (EGP)</div><div class="kpi-value">${fmtpresent(totalEGP)}</div></div>
                <div><div class="kpi-label" data-i18n="foreign_currency_value">Foreign Currency Value</div><div class="kpi-value">${fmtpresent(usdAmount * usdRate + eurAmount * eurRate + sarAmount * sarRate)}</div></div>
                <div><div class="kpi-label" data-i18n="gold_value">Gold Value</div><div class="kpi-value">${fmtpresent(goldValue)}</div></div>
                <div><div class="kpi-label">Monthly Salary</div><div class="kpi-value">${fmtpresent(forecastData.monthly_salary || 0)}</div></div>
                <div><div class="kpi-label">Certificate Income</div><div class="kpi-value">${fmtpresent(forecastData.monthly_certificate_income || 0)}</div></div>
        ...
        <div class="kpi-card mb-4">
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:16px; margin-top:20px;">
                <div><div class="kpi-label" data-i18n="next_30_days">Next 30 Days</div><div class="kpi-value">${fmtpresent(forecastData.forecast_30 || 0)}</div></div>
                <div><div class="kpi-label" data-i18n="next_90_days">Next 90 Days</div><div class="kpi-value">${fmtpresent(forecastData.forecast_90 || 0)}</div></div>
                <div><div class="kpi-label" data-i18n="next_180_days">Next 180 Days</div><div class="kpi-value">${fmtpresent(forecastData.forecast_180 || 0)}</div></div>
            </div>
        </div>
        ...
        <div class="kpi-card mb-4">
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:16px; margin-top:20px;">
                <div><div class="kpi-label" data-i18n="current_cash">Current Cash</div><div class="kpi-value">${fmtpresent(forecastData.cash_balance || 0)}</div></div>
                <div><div class="kpi-label" data-i18n="cash_after_30_days">Cash After 30 Days</div><div class="kpi-value">${fmtpresent(forecastData.future_cash_30 || 0)}</div></div>
                <div><div class="kpi-label" data-i18n="cash_after_90_days">Cash After 90 Days</div><div class="kpi-value">${fmtpresent(forecastData.future_cash_90 || 0)}</div></div>
                <div><div class="kpi-label" data-i18n="cash_after_180_days">Cash After 180 Days</div><div class="kpi-value">${fmtpresent(forecastData.future_cash_180 || 0)}</div></div>
            </div>
        </div>
        ...
        <div class="kpi-card mb-4">
            ...
            <div style="margin-top:15px">
                ${(forecastData.recommendations || []).map((r) => `<div style="padding:10px; margin-bottom:8px; border-radius:8px; background:var(--bg-secondary); border:1px solid var(--border-color);">${r}</div>`).join("")}
            </div>
        </div>

        <div class="kpi-card mb-4">
            <div class="kpi-label" data-i18n="recommended_action">Recommended Action</div>
            <div style="margin-top: 15px; font-weight: 600;">${forecastData.action_plan || ""}</div>
        </div>
        ...
        ${forecastData.upcoming?.length ? `
            ...
            <div class="table-container" style="margin-top:15px; margin-bottom: 24px;">
                <table class="data-table">
                    ...
                    <tbody>
                        ${forecastData.upcoming.map((c) => `<tr><td>${c.bank}</td><td class="local-date-field" data-expiry="${c.expiry_date}"></td><td><span style="color:${c.days_left <= 30 ? "var(--accent-red)" : c.days_left <= 90 ? "orange" : "var(--text-primary)"}; font-weight:600;">${c.days_left}</span></td><td>${fmtpresent(c.amount)}</td></tr>`).join("")}
                    </tbody>
                </table>
            </div>` : ""
        }
        ...
        <div class="kpi-card mb-4">
            ...
            <div style="margin-top:15px">
                ${(forecastData.recommendations || []).map((r) => `<div style="padding:12px; margin-bottom:10px; background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:10px;">${r}</div>`).join("")}
            </div>
        </div>
