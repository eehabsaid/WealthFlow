...
function showCardRenewalFeeModal(id = null) {
    ...
    const titleText  = isEdit ? t('edit_card_renewal_fee', 'Edit Card Renewal Fee') : t('new_card_renewal_fee', 'New Card Renewal Fee');
    ...
    const bankOptions = _banks.map(b => `<option value="${b.id}" ${crf.bank_id === b.id ? 'selected' : ''}>${b.name}</option>`).join('');
