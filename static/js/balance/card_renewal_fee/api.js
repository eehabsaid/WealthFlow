...
async function saveCardRenewalFee() {
    ...
    try {
        const url = _editingCardRenewalFeeId ? `/api/card-renewal-fees/${_editingCardRenewalFeeId}/` : '/api/card-renewal-fees/';
        ...
        if (!res.ok) {
            ...
            const errMsg = err.error ? t(err.error, err.error) : t('error_failed_to_save', 'Failed to save card renewal fee');
