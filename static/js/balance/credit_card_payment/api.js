...
async function saveCreditCardPayment() {
    ...
    try {
        const url = _editingCreditCardPaymentId ? `/api/credit-card-payments/${_editingCreditCardPaymentId}/` : '/api/credit-card-payments/';
        ...
        if (!res.ok) {
            ...
            const errMsg = err.error ? t(err.error, err.error) : t('error_failed_to_save', 'Failed to save credit card payment');
