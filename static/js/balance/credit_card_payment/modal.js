...
function showCreditCardPaymentModal(id = null) {
  ...
  const titleText = isEdit
    ? t("edit_credit_card_payment", "Edit Credit Card Payment")
    : t("new_credit_card_payment", "New Credit Card Payment");
  ...
  const bankOptions = _banks
    .map(
      (b) => `<option value="${b.id}" ${cp.bank_id === b.id ? "selected" : ""}>${b.name}</option>`
    )
    .join("");

  const methodOptions = ["Card", "Bank Transfer"]
    .map(
      (m) =>
