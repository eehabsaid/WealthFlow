...
function showBankInterestModal(id = null) {
    ...
    const titleText  = isEdit ? t('edit_bank_interest', 'Edit Bank Interest') : t('new_bank_interest', 'New Bank Interest');
    ...
    const bankOptions = _banks.map(b => `<option value="${b.id}" ${bi.bank_id === b.id ? 'selected' : ''}>${b.name}</option>`).join('');
