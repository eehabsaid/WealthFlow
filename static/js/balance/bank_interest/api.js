...
async function saveBankInterest() {
    ...
    try {
        const url = _editingBankInterestId ? `/api/bank-interests/${_editingBankInterestId}/` : '/api/bank-interests/';
        ...
        if (!res.ok) {
            ...
            const errMsg = err.error ? t(err.error, err.error) : t('error_failed_to_save', 'Failed to save bank interest');
