...
async function renderBalanceCurrencyExchange(data) {
    ...
    if (_currencyExchangesData.length === 0) {
        ...
        rowsHtml = _currencyExchangesData.map(ex => {
            const statusLabel = ex.status === 'ACTIVE' ? t('status_active', 'Active') 
                : (ex.status === 'REVERSED' ? t('status_reversed', 'Reversed') : t('status_edited', 'Edited'));
...
async function showExchangeModal(id = null) {
    ...
    const titleText = isEdit ? t('edit_exchange', 'Edit Currency Exchange') : t('new_exchange', 'New Currency Exchange');
    const cancelText = t('btn_cancel', 'Cancel');
    const saveText = t('btn_save', 'Save');

    const balances = _exchangeFormOptions.balances || [];
    const sourceOptions = balances.map(b => 
        `<option value="${b.id}">${getTranslatedBalanceTitle(b)}</option>`
    ).join('');
...
async function calculateBackendExchange(isUserRateChange = false) {
    ...
    try {
        ...
        if (toCurrEl && calc.to_currency_code) toCurrEl.value = `${calc.to_currency_flag} ${calc.to_currency_code}`;
...
async function saveExchange() {
    ...
    try {
        const url = _editingExchangeId ? `/api/currency-exchanges/${_editingExchangeId}/` : '/api/currency-exchanges/';
        ...
        if (!res.ok) {
            ...
            const errMsg = err.error ? t(err.error, err.error) : t('error_failed_to_save', 'Failed to save exchange');
...
async function deleteExchange(id) {
    if (!confirm(t('confirm_delete', 'Are you sure you want to delete this exchange transaction? It will be reversed automatically.'))) return;
    try {
...
function filterCurrencyExchanges() {
    ...
    const filtered = _currencyExchangesData.filter(ex => {
        ...
        const text = `${ex.from_balance_title} ${ex.to_balance_title} ${ex.from_currency_code} ${ex.to_currency_code} ${ex.notes}`.toLowerCase();
...
function resetCurrencyExchangeFilters() {
    if (document.getElementById('ce_filter_search')) document.getElementById('ce_filter_search').value = '';
    if (document.getElementById('ce_filter_status')) document.getElementById('ce_filter_status').value = 'ALL';
    if (document.getElementById('ce_filter_date')) document.getElementById('ce_filter_date').value = '';
