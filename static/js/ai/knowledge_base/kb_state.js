...
window.KB.CATEGORIES = [
  ...
  { code: 'codebase_architecture', i18n: 'ai_kb_cat_codebase', fallback: 'Codebase & Architecture' },
...
window.KB.loadEntries = async function () {
  ...
  try {
    const category = state.activeCategory === 'all' ? '' : `?category=${encodeURIComponent(state.activeCategory)}`;
