...
async function renderDashboard() {
    ...
    mc.innerHTML = `
        ...
        <div class="row g-3 mb-4">
            ...
            ${kpiCard(t('kpi_total_remaining', 'Total Remaining'), fmt(g.total_remaining), 'bi-hourglass-split', g.total_remaining > 0 ? 'var(--accent-red)' : 'var(--text-muted)')}
        ...
        <div style="background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:12px;overflow:hidden">
            <table class="data-table">
                ...
                <tfoot><tr class="total-row">
                    ...
                    <td class="text-end">${fmt(g.total_remaining)}</td>
                </tr></tfoot>
            </table>
        </div>`;
    applyTranslations();
...
function renderYearPills(years, activeYear, companyId) {
    ...
    container.innerHTML = years.map(y => `
        <button class="year-pill ${y === activeYear ? 'active' : ''}"
            onclick="switchYear(${y}, ${companyId})">${y}</button>`).join('');
}
...
function renderSalaryTable(allEntries, year, companyId) {
    ...
    let totExp = 0, totPaid = 0, totBonus = 0;
    entries.forEach(e => { totExp += e.expected; totPaid += e.paid; totBonus += e.bonus; });
        ...
        <div style="background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:12px;overflow:hidden">
            <table class="data-table">
                ...
                <tfoot><tr class="total-row">
                    ...
                    <td class="text-end ${amtClass(totExp - totPaid)}">${fmt(totExp - totPaid)}</td>
                    <td></td>
                </tr></tfoot>
            </table>
        </div>`;
...
async function showSalaryModal(entryId, companyId) {
    ...
    const months = ['January', 'February', 'March', 'April', 'May', 'June',
        ...
        'Quarter-Bonuses'];
    const opts = months.map(m => `<option value="${m}" ${entry && entry.month === m ? 'selected' : ''}>${m}</option>`).join('');
...
async function deleteSalaryEntry(entryId, companyId) {
    ...
    if (res.ok) { showToast('Deleted'); renderSalaryPage(companyId); }
}