"use strict";
// Builds the fixed-asset save payload from form fields, including
// type-specific details and per-row bank-account validation.
//
// Behavior notes preserved exactly from the original monolithic
// saveFixedAsset (phase-function split, 200-line rule):
// - Returns null if purchase-payment validation fails (already toasted via
//   showToast) so the caller can silently abort, matching the original
//   early `return`.
// - The money-movement bank-account validation loop throws on failure. In
//   the original this ran BEFORE showLoading()/try, so a thrown error here
//   is NOT caught locally by saveFixedAsset's try/catch — it propagates
//   uncaught, same as before. Do not wrap this call in the caller's try
//   block, or that behavior changes.
//
// Part of the fixed_assets module. Do not edit directly.

function buildFixedAssetPayload() {
